%% optm_start_line_search - Intiate a new line-search.
%%
...
%% which is updated to reflec that a new line search has been started.  The
...
%% to try.
%%
%% Upon return of `optm_start_line_search`, the line-search context `lnsrch`
%% is updated so that `lnsrch.state` is normally equal to 1 indicating that the
%% next step to try is `lnsrch.step` (which should be equal to `stp` in that
