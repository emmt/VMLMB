...
%%
%% to creates a new line-search context.  All parameters are specified by their
...
%%   step is computed based on a parabolic interpolation of the objective
%%   function along the search direction.  The new step writes:
%%
...
%%   emulates the usual Armijo's method.  Default values are `smin = 0.1` and
%%   `smax = 0.9`.
%%
%% Note that if `smin >= 1/(2 - 2*ftol)`, then `gamma` will always be equal
%% to `smin`.  Therefore, to benefit from quadratic interpolation, one should
